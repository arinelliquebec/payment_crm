...
--     anyone can log in.
--
-- Schema target: PostgreSQL, generated by the EF Core migration
-- `InitialCreatePg`. The set of tables & columns below mirrors that schema.
--
-- Safety properties:
...
--   * Only INSERTs into the listed tables. No DDL on application tables.
--   * Idempotent: re-running is safe (ON CONFLICT DO NOTHING).
--
-- How to run (only after explicit human approval — see seed/README.md):
--   psql -d payment_crm_demo -v ON_ERROR_STOP=1 -f seed/seed-demo.sql
-- or, preferred wrapper:
...
INSERT INTO "Enderecos"
  ("Id","Cidade","Estado","Bairro","Logradouro","Cep","Numero","Complemento")
SELECT
  10100000 + n,
  CASE (n % 4)
...
-- ----------------------------------------------------------------------------
-- 4. PessoasFisicas (40; ids 10200001..10200040)  UNIQUE(Cpf), UNIQUE(EmailEmpresarial)
...
SELECT
  ...
  CASE (n % 3) WHEN 0
       THEN 'demo.pf.pessoal+' || lpad(n::text, 3, '0') || '@example.org'
       ELSE NULL END,
  '[DEMO] Codinome '      || lpad(n::text, 3, '0'),
  CASE (n % 2) WHEN 0 THEN 'F' ELSE 'M' END,
  (date '1980-01-01' + ((n * 73) % 10000))::timestamptz,
  CASE (n % 4)
...
-- ----------------------------------------------------------------------------
-- 5. PessoasJuridicas (15; ids 10300001..10300015)  UNIQUE(Cnpj)
...
-- ----------------------------------------------------------------------------
-- 6. Consultores (5; ids 10400001..10400005)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 7. Parceiros (3; ids 10500001..10500003)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 8. Usuarios (6; ids 10600001..10600006)  UNIQUE(Login)
...
--    GrupoAcessoId/FilialId/ConsultorId left NULL: the application seeds the
--    GruposAcesso/Permissoes catalogue on startup and admin assigns later.
-- ----------------------------------------------------------------------------

INSERT INTO "Usuarios"
  ("Id","Login","Email","Senha","GrupoAcessoId","FilialId","ConsultorId",
   "TipoPessoa","PessoaFisicaId","PessoaJuridicaId","Ativo",
   "DataCadastro","DataAtualizacao","UltimoAcesso")
VALUES
  (10600001,'demo-admin',      'demo-admin@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200009, NULL, true, now() - interval '120 days', NULL, NULL),
  (10600002,'demo-gestor',     'demo-gestor@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200010, NULL, true, now() - interval '120 days', NULL, NULL),
  (10600003,'demo-consultor',  'demo-consultor@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200011, NULL, true, now() - interval '120 days', NULL, NULL),
  (10600004,'demo-financeiro', 'demo-financeiro@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200012, NULL, true, now() - interval '120 days', NULL, NULL),
  (10600005,'demo-faturamento','demo-faturamento@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200013, NULL, true, now() - interval '120 days', NULL, NULL),
  (10600006,'demo-leitura',    'demo-leitura@example.com',
   '__DEMO_PLACEHOLDER_RESET_BEFORE_USE__', NULL, NULL, NULL,
   'PF', 10200014, NULL, true, now() - interval '120 days', NULL, NULL)
...
INSERT INTO "Clientes"
  ("Id","TipoPessoa","PessoaFisicaId","PessoaJuridicaId","ConsultorAtualId","FilialId",
   "Status","Observacoes","EmailAlternativo",
   "DataCadastro","DataAtualizacao","Ativo")
SELECT
  10700000 + n,
  ...
  CASE (n % 5) WHEN 0
       THEN 'demo.cliente.alt+' || lpad(n::text, 3, '0') || '@example.org'
       ELSE NULL END,
...
--     `Situacao` and `MetodoPagamento` are NOT NULL in the current schema.
-- ----------------------------------------------------------------------------
...
INSERT INTO "Contratos"
  ("Id","ClienteId","ConsultorId","ParceiroId","Situacao",
   ...
   "AnexoDocumento","Pendencias","MetodoPagamento",
   "DataCadastro","DataAtualizacao","Ativo")
SELECT
  10900000 + ((c - 1) * 2 + k),
  10700000 + c,
  ...
  CASE WHEN ((c + k) % 7) = 0
       ...
       ELSE NULL END,
  CASE ((c + k) % 3) WHEN 0 THEN 'Boleto' WHEN 1 THEN 'PIX' ELSE 'Transferência' END,
